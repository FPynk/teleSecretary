...
class TelegramUrgentCommandTests(unittest.IsolatedAsyncioTestCase):
    async def test_urgent_command_reads_only_the_authenticated_owners_tasks(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            updates = (
                self.build_update(telegram_user_id=1001, text="/urgent"),
                self.build_update(telegram_user_id=1001, text="/URGENT"),
                self.build_update(telegram_user_id=1001, text="/urgent@TeleSecretaryBot"),
            )
            for update in updates:
                await _urgent_handler(config)(update, SimpleNamespace())

        for update in updates:
            self.assertIn("Owner urgent task", update.message.replies[0])
            self.assertNotIn("Private urgent task", update.message.replies[0])

    async def test_urgent_rejects_arguments_and_unauthorized_callers_before_database_access(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            config = self.build_config(Path(temp_dir))
            invalid_update = self.build_update(telegram_user_id=1001, text="/urgent now")
            unauthorized_update = self.build_update(telegram_user_id=2002, text="/urgent")

            await _urgent_handler(config)(invalid_update, SimpleNamespace())
            await _urgent_handler(config)(unauthorized_update, SimpleNamespace())

        self.assertEqual(invalid_update.message.replies, ["Usage: /urgent"])
        self.assertEqual(
    ...
    def test_urgent_parser_help_and_application_registration(self) -> None:
        self.assertTrue(parse_urgent_command_text("/urgent"))
        self.assertTrue(parse_urgent_command_text("/URGENT@TeleSecretaryBot"))
        self.assertFalse(parse_urgent_command_text("/urgent today"))
        self.assertIn("/urgent - show high-priority tasks", build_help_response())
        ...
        self.assertIn("urgent", [handler.command for handler in application.handlers])
